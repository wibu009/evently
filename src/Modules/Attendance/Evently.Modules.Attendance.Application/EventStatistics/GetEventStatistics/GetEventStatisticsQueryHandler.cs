    ...
    : IQueryHandler<GetEventStatisticsQuery, EventStatisticsResponse>
{
    public async Task<Result<EventStatisticsResponse>> Handle(GetEventStatisticsQuery request, CancellationToken cancellationToken)
    {
        ...
        const string sql =
            $"""
             SELECT
                 ...
                 start_at_utc AS {nameof(EventStatisticsResponse.StartAtUtc)},
                 end_at_utc AS {nameof(EventStatisticsResponse.EndAtUtc)},
        ...
        EventStatisticsResponse? eventStatistics = await connection.QuerySingleOrDefaultAsync<EventStatisticsResponse>(sql, request);
